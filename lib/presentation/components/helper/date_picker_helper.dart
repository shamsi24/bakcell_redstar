...
import 'package:redstar_module/presentation/components/picker/date_picker.dart';
    ...
    if (selectedDate != null) {
      ...
      if (calendar == Calendar.startDate) {
        ...
        if (DateTime.parse(dateStr).isBefore(DateTime.parse(_startDate))) {
          throw Failure(ErrorMessage(
              message: "Son tarix, başlanğıc tarixdən kiçik ola bilməz."));
        }
        _endDate = dateStr;
      } else if (calendar == Calendar.orderDate) {
        if (DateTime.parse(dateStr).isBefore(DateTime.now())) {
          throw Failure(
              ErrorMessage(message: "Keçmiş tarixi seçə bilməzsiniz."));
        }
        _orderDate = dateStr;
      }
      return dateStr;
